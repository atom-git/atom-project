<template>
  <a-layout class="atom-form-maker">
    ...
    <!-- 中间画布面板区域 -->
    <MakerPanel :makerConfig="makerConfig" @maker-widget-change="handleWidgetChange"></MakerPanel>
    <!-- 右侧配置面板区域 -->
...
export default {
  ...
  computed: {
    // 配置字段
    configFields () {
      return (this.curWidget && this.curWidget.fields) || []
    }
  },
  methods: {
    // 响应当前填加的组件改变
    ...
    handleWidgetConfigChange (widgetConfig) {
      this.curWidget.config = { ...widgetConfig }
    }
  }
}
</script>

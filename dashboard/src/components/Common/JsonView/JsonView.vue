<template>
  <div :class="jvClass">
    ...
    <div class="jv-code" :class="{ 'open': expandCode, boxed }">
      <JsonBox ref="jsonBox" :value="value" :sort="sort" :preview-mode="previewMode"/>
    </div>
    <div v-if="expandableCode && boxed" class="jv-more" @click="toggleExpandCode">
      <IconFont :type="expandCode ? 'CaretRightOutlined' : 'CaretDownOutlined'" />
    </div>
  </div>
</template>
...
export default {
  ...
  mounted: function () {
    this.debounceResized = this.$utils.debounce(this.debResized.bind(this), 200);

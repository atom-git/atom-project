<template>
  <a-form class="atom-sign-form" ref="forgetForm" :model="forgetUser" :rules="forgetRules">
    ...
    <a-form-item name="phone">
      <a-input v-model:value="forgetUser.phone" placeholder="请输入手机号" size="large" allowClear>
        <template #prefix>
    ...
    <a-form-item name="verifyCode" class="atom-form-flex">
      <a-input v-model:value="forgetUser.verifyCode" placeholder="请输入验证码" size="large" allowClear>
        <template #prefix>
...
export default {
  ...
  methods: {
    ...
    sendVerifyCode () {
      // 调用后台短信网关
      this.$api.system.sendVerifyCode(this.forgetUser.phone).then(() => {
